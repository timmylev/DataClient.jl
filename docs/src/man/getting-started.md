...
Notice that there are two types of stores available, [`DataClient.S3DB`](@ref) and [`DataClient.FFS`](@ref).
...
The `datafeeds-arrow` store, on the other hand, is generated by the [S3DBConverter](https://gitlab.invenia.ca/invenia/Datafeeds/S3DBConverter) which takes the outputs of `datafeeds` and converts them into `arrow.zst`-formatted files, a more optimized format for our use case.
There is no different between the data retrieved from the two stores.
...
While DataClient supports reloading configs (eg. used in [`reload_backend`](@ref)), re-initializing the cache is not currently supported.
```julia
